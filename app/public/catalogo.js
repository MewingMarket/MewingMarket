/* =========================================================
   CATALOGO — Versione 2058 (Single Loader Architecture)
   - Nessun autorun
...
console.log("📌 [CATALOGO 2058] File caricato");

/* =========================================================
   PAGE INIT — chiamata da Loader Supremo 2058
========================================================= */
window.pageInit = function () {
  console.log("🏁 [CATALOGO 2058] pageInit() avviata");
  avviaIlCatalogoOra();
};
...
function cardHTML(p) {
  ...
  const countdownHtml = p.promo_scadenza
    ? `<p class="promo-countdown" data-scadenza="${p.promo_scadenza}"></p>`
    : "";

  const prezzoHtml = hasPromo
    ? `<p class="prezzo"><span class="prezzo-originale">€${prezzoBaseEuro}</span><span class="prezzo-scontato">€${prezzoPromoEuro}</span></p>`
    : `<p class="prezzo">€${prezzoBaseEuro}</p>`;

  const prezzoCarrello = hasPromo ? p.prezzo_scontato_cent : p.prezzo_cent;
        ...
        <div class="card-buttons">
          ...
          <div class="cart-controls">
            <button class="btn-add-cart"
              onclick="window.aggiungiAlCarrello({id:'${id}', titolo:'${titolo}', prezzo_cent:${prezzoCarrello}, immagine:'${img}'})">+</button>
...
/* =========================================================
   API WRAPPER
========================================================= */
async function apiCatalogo(path, payload = {}) {
  try {
...
async function caricaCatalogoBase() {
  const res = await apiCatalogo("/api/prodotti-new");
...
async function caricaCatalogoPersonalizzato() {
  ...
  if (!res.success || !Array.isArray(res.prodotti)) return null;
  return res.prodotti.some(p => p.promo_attiva) ? res.prodotti : null;
}
...
async function avviaIlCatalogoOra() {
  if (__CATALOGO_GIA_CARICATO__) {
    console.log("♻️ [CATALOGO] Catalogo già caricato");
    return;
  }
  ...
  try {
    ...
    if (!isLogged) {
      ...
      if (!hasPromo) {
        ...
        prodotti =
          personalizzato && personalizzato.length
            ? personalizzato
            : await caricaCatalogoBase();
      }
    }
  } catch (err) {
